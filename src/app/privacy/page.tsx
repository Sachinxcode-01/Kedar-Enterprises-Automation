'use client';

import React, { useState } from 'react';
import { motion } from 'framer-motion';
import {
  initialConsentRecords,
  initialDataRequests,
  initialGrievanceTickets,
  initialVendors,
  initialRetentionPolicies,
} from '@/lib/data';
import {
  ConsentRecord,
  DataRequest,
  GrievanceTicket,
  Vendor,
  RetentionPolicy,
} from '@/lib/types';
import {
  ShieldCheck,
  FileText,
  CheckCircle2,
  Trash2,
  AlertTriangle,
  Server,
  Clock,
  HelpCircle,
  Sparkles,
  Download,
  Lock,
  Search,
} from 'lucide-react';

export default function PrivacyPage() {
  const [activeTab, setActiveTab] = useState<
    'NOTICE' | 'CONSENT' | 'REQUESTS' | 'GRIEVANCES' | 'VENDORS' | 'RETENTION'
  >('NOTICE');

  const [consents, setConsents] = useState<ConsentRecord[]>(initialConsentRecords);
  const [requests, setRequests] = useState<DataRequest[]>(initialDataRequests);
  const [tickets, setTickets] = useState<GrievanceTicket[]>(initialGrievanceTickets);
  const [vendors, setVendors] = useState<Vendor[]>(initialVendors);
  const [retention, setRetention] = useState<RetentionPolicy[]>(initialRetentionPolicies);

  const resolveTicket = (id: string) => {
    setTickets((prev) =>
      prev.map((t) =>
        t.id === id
          ? { ...t, status: 'RESOLVED', resolution: 'Resolved by DPDP Compliance Officer.' }
          : t
      )
    );
  };

  return (
    <div className="space-y-8 max-w-7xl mx-auto pb-10">
      {/* Top Banner */}
      <div className="p-6 lg:p-8 rounded-3xl bg-gradient-to-r from-emerald-950/80 via-slate-900 to-slate-900 border border-emerald-800/40 shadow-2xl flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <div className="flex items-center gap-2 text-xs font-bold text-emerald-400 uppercase tracking-wider mb-2 font-mono">
            <ShieldCheck className="w-4 h-4" />
            <span>Digital Personal Data Protection (DPDP) Act, 2023 Controls</span>
          </div>
          <h2 className="text-2xl lg:text-3xl font-black text-white tracking-tight">
            Privacy-by-Design & Compliance Management Center
          </h2>
          <p className="text-slate-300 text-xs sm:text-sm mt-1 max-w-3xl leading-relaxed">
            Architectural compliance controls for Data Principal Rights, Purpose Limitation, Consent Ledger, Grievances, Retention Purges, and Processor Inventory under Indian regulations.
          </p>
        </div>

        <div className="px-4 py-2.5 rounded-2xl bg-emerald-900/40 border border-emerald-600/40 text-emerald-300 text-xs font-bold flex items-center gap-2 shrink-0">
          <Lock className="w-4 h-4 text-emerald-400" />
          <span>DPDP Statutory Engine Active</span>
        </div>
      </div>

      {/* Sub Tabs */}
      <div className="flex items-center gap-1.5 bg-slate-900/80 p-1.5 rounded-2xl border border-slate-800 overflow-x-auto text-xs font-bold font-mono">
        <button
          onClick={() => setActiveTab('NOTICE')}
          className={`px-4 py-2 rounded-xl whitespace-nowrap transition-all ${
            activeTab === 'NOTICE' ? 'bg-emerald-600 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'
          }`}
        >
          Privacy Notice
        </button>
        <button
          onClick={() => setActiveTab('CONSENT')}
          className={`px-4 py-2 rounded-xl whitespace-nowrap transition-all ${
            activeTab === 'CONSENT' ? 'bg-emerald-600 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'
          }`}
        >
          Consent Ledger ({consents.length})
        </button>
        <button
          onClick={() => setActiveTab('REQUESTS')}
          className={`px-4 py-2 rounded-xl whitespace-nowrap transition-all ${
            activeTab === 'REQUESTS' ? 'bg-emerald-600 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'
          }`}
        >
          Data Principal Requests ({requests.length})
        </button>
        <button
          onClick={() => setActiveTab('GRIEVANCES')}
          className={`px-4 py-2 rounded-xl whitespace-nowrap transition-all ${
            activeTab === 'GRIEVANCES' ? 'bg-emerald-600 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'
          }`}
        >
          Grievance Redressal ({tickets.length})
        </button>
        <button
          onClick={() => setActiveTab('VENDORS')}
          className={`px-4 py-2 rounded-xl whitespace-nowrap transition-all ${
            activeTab === 'VENDORS' ? 'bg-emerald-600 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'
          }`}
        >
          Processors Inventory ({vendors.length})
        </button>
        <button
          onClick={() => setActiveTab('RETENTION')}
          className={`px-4 py-2 rounded-xl whitespace-nowrap transition-all ${
            activeTab === 'RETENTION' ? 'bg-emerald-600 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'
          }`}
        >
          Retention Schedules
        </button>
      </div>

      {/* TAB 1: PRIVACY NOTICE */}
      {activeTab === 'NOTICE' && (
        <div className="glass-panel p-6 lg:p-8 rounded-3xl space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-base font-bold text-white flex items-center gap-2">
              <FileText className="w-4 h-4 text-emerald-400" /> Active WhatsApp Transparency Notice (v1.0-2024)
            </h3>
            <span className="text-xs text-emerald-400 font-mono font-bold">Published & Live on Webhook</span>
          </div>

          <div className="p-5 rounded-2xl bg-slate-950 border border-slate-800 text-xs text-slate-300 space-y-3.5 leading-relaxed">
            <p>
              <strong>Kedar Enterprises Statutory Privacy Notice for WhatsApp Customers:</strong>
            </p>
            <p>
              Under the Digital Personal Data Protection (DPDP) Act, 2023, Kedar Enterprises collects your name, WhatsApp phone number, location, and inquiry requirements strictly for responding to business inquiries, providing product quotations, and managing commercial service orders.
            </p>
            <ul className="list-disc pl-5 space-y-1.5 text-slate-400">
              <li><strong>Data Fiduciary:</strong> Kedar Enterprises, Plot 45, Industrial Suburb Stage II, Bengaluru, Karnataka.</li>
              <li><strong>Purpose Limitation:</strong> Data is never sold or repurposed for unsolicited third-party marketing.</li>
              <li><strong>Data Principal Rights:</strong> You may request access, correction, or erasure of your personal data at any time by messaging &quot;PRIVACY&quot; or contacting privacy@kedarenterprises.com.</li>
            </ul>
          </div>
        </div>
      )}

      {/* TAB 2: CONSENT LEDGER */}
      {activeTab === 'CONSENT' && (
        <div className="glass-panel rounded-3xl overflow-hidden shadow-2xl">
          <table className="w-full text-left text-xs text-slate-300">
            <thead className="bg-slate-950/80 border-b border-slate-800 text-[11px] font-bold text-slate-400 uppercase font-mono">
              <tr>
                <th className="p-4">Customer</th>
                <th className="p-4">Phone Number</th>
                <th className="p-4">Consent Purpose</th>
                <th className="p-4">Consent Status</th>
                <th className="p-4">Notice Version</th>
                <th className="p-4">Timestamp</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-800/60">
              {consents.map((cs) => (
                <tr key={cs.id} className="hover:bg-slate-800/40 transition-colors">
                  <td className="p-4 font-bold text-white">{cs.customer_name}</td>
                  <td className="p-4 font-mono text-slate-300">{cs.customer_phone}</td>
                  <td className="p-4 max-w-xs">{cs.purpose}</td>
                  <td className="p-4">
                    {cs.status === 'GRANTED' ? (
                      <span className="px-2.5 py-1 rounded-full bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 text-[10px] font-bold">
                        GRANTED
                      </span>
                    ) : (
                      <span className="px-2.5 py-1 rounded-full bg-rose-500/20 text-rose-400 border border-rose-500/30 text-[10px] font-bold">
                        WITHDRAWN
                      </span>
                    )}
                  </td>
                  <td className="p-4 font-mono text-slate-400">{cs.notice_version}</td>
                  <td className="p-4 text-slate-400 font-mono">{cs.granted_at}</td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      )}

      {/* TAB 3: DATA PRINCIPAL REQUESTS */}
      {activeTab === 'REQUESTS' && (
        <div className="glass-panel rounded-3xl overflow-hidden shadow-2xl">
          <table className="w-full text-left text-xs text-slate-300">
            <thead className="bg-slate-950/80 border-b border-slate-800 text-[11px] font-bold text-slate-400 uppercase font-mono">
              <tr>
                <th className="p-4">Customer</th>
                <th className="p-4">Request Type</th>
                <th className="p-4">Details & Compliance Ground</th>
                <th className="p-4">Execution Status</th>
                <th className="p-4">Requested At</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-800/60">
              {requests.map((r) => (
                <tr key={r.id} className="hover:bg-slate-800/40 transition-colors">
                  <td className="p-4 font-bold text-white">{r.customer_name}</td>
                  <td className="p-4">
                    <span className="px-2.5 py-1 rounded-full bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 font-bold text-[10px] font-mono">
                      {r.request_type}
                    </span>
                  </td>
                  <td className="p-4 max-w-sm text-slate-400">{r.details}</td>
                  <td className="p-4 font-mono font-bold text-emerald-400">{r.status}</td>
                  <td className="p-4 text-slate-400 font-mono">{r.requested_at}</td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      )}

      {/* TAB 4: GRIEVANCES */}
      {activeTab === 'GRIEVANCES' && (
        <div className="space-y-4">
          {tickets.map((t) => (
            <div key={t.id} className="glass-panel p-6 rounded-3xl flex items-start justify-between gap-4">
              <div className="space-y-1.5">
                <div className="flex items-center gap-2">
                  <span className="font-mono text-xs text-brand-400 font-bold">{t.ticket_number}</span>
                  <span className="font-bold text-sm text-white">{t.customer_name}</span>
                </div>
                <p className="text-xs text-slate-300 leading-relaxed">{t.issue}</p>
                <div className="text-[10px] text-slate-500 font-mono">Category: {t.category} • Created: {t.created_at}</div>
              </div>
              <div className="shrink-0">
                {t.status === 'OPEN' ? (
                  <button
                    onClick={() => resolveTicket(t.id)}
                    className="px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs shadow-md transition-all"
                  >
                    Mark Resolved
                  </button>
                ) : (
                  <span className="px-3 py-1 rounded-full bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 text-xs font-bold font-mono">
                    RESOLVED
                  </span>
                )}
              </div>
            </div>
          ))}
        </div>
      )}

      {/* TAB 5: VENDORS / PROCESSORS */}
      {activeTab === 'VENDORS' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {vendors.map((v) => (
            <div key={v.id} className="glass-panel p-6 rounded-3xl space-y-2.5">
              <div className="flex items-center justify-between">
                <h4 className="font-bold text-sm text-white">{v.vendor_name}</h4>
                <span className="px-2.5 py-0.5 text-[10px] font-bold rounded-full bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 font-mono">
                  {v.contract_status}
                </span>
              </div>
              <p className="text-xs text-brand-400 font-semibold">{v.service_provided}</p>
              <p className="text-xs text-slate-400 leading-relaxed">Data Shared: {v.data_shared}</p>
              <div className="pt-2.5 border-t border-slate-800 text-[10px] text-slate-500 font-mono flex justify-between">
                <span>Storage: {v.data_location}</span>
                <span>Audit: {v.last_security_audit}</span>
              </div>
            </div>
          ))}
        </div>
      )}

      {/* TAB 6: DATA RETENTION */}
      {activeTab === 'RETENTION' && (
        <div className="glass-panel rounded-3xl overflow-hidden shadow-2xl">
          <table className="w-full text-left text-xs text-slate-300">
            <thead className="bg-slate-950/80 border-b border-slate-800 text-[11px] font-bold text-slate-400 uppercase font-mono">
              <tr>
                <th className="p-4">Data Category</th>
                <th className="p-4">Retention Period</th>
                <th className="p-4">Automated Purge</th>
                <th className="p-4">Statutory Basis</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-800/60">
              {retention.map((rp) => (
                <tr key={rp.id} className="hover:bg-slate-800/40 transition-colors">
                  <td className="p-4 font-bold text-white">{rp.data_category}</td>
                  <td className="p-4 font-bold text-brand-400 font-mono">{rp.retention_period_days} Days</td>
                  <td className="p-4">
                    {rp.auto_purge ? (
                      <span className="px-2.5 py-1 rounded-full bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 font-bold text-[10px] font-mono">
                        ENABLED
                      </span>
                    ) : (
                      <span className="px-2.5 py-1 rounded-full bg-slate-800 text-slate-400 border border-slate-700 font-bold text-[10px] font-mono">
                        MANUAL REVIEW
                      </span>
                    )}
                  </td>
                  <td className="p-4 text-slate-400">{rp.legal_basis}</td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      )}
    </div>
  );
}
